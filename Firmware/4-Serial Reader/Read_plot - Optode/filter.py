...
for peakInt in range(1,peakNum+1):
  Left =  randint(limit,min(limit+2000,min(HbRNum-1,HbONum-1)))
  peak = Left + randint(500,1000)
  Right = peak+ randint(500,1000)
  print("Peak Number : "+str(peakInt),[Left,peak,Right])
  
  if (peakInt%2==randint(0,1)):
    for i in range(Left,Right):
      specialHbOVal[min(i,HbONum-1)] += int(-(randint(75,100)/1000.0)*((i-Left)*(i-Right)/(Left+Right)+randint(-10,10)))
    for i in range(Left,Right):
      specialHbRVal[min(i+randint(10,20),HbRNum-1)] += int(+(randint(75,100)/1000.0)*((i-Left)*(i-Right)/(Left+Right)+randint(-10,10)))
